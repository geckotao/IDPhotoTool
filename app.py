...
from PyQt6.QtWidgets import (
    ...
    QMessageBox, QGridLayout, QDial, QColorDialog, QProgressBar,
...
def intelligent_crop_id_photo(image, target_width, target_height,
    ...
    if face_rotate_angle != 0 and face_info is not None:
        # 以人脸中心为旋转中心进行校正
        center = (face_info["center_x"], face_info["center_y"])
        processed_image = rotate_bound(image, face_rotate_angle, center=center)
        
        # 旋转后图像尺寸变化，需要重新检测人脸位置
        # 因为旋转后坐标系统已改变，必须重新检测
        if face_detector is not None:
            face_info = face_detector.detect_face(processed_image[:, :, :3])
            print(f"🔄 旋转 {face_rotate_angle}° 后重新检测人脸：{face_info['box'] if face_info else '未检测到'}")
    ...
    if face_info is not None:
        ...
        # 无人脸检测时，中心裁剪
        crop_width = min(processed_image.shape[1], int(processed_image.shape[0] * target_width / target_height))
        crop_height = min(processed_image.shape[0], int(processed_image.shape[1] * target_height / target_width))
        crop_x1 = (processed_image.shape[1] - crop_width) // 2
        crop_y1 = (processed_image.shape[0] - crop_height) // 2
        crop_x2 = crop_x1 + crop_width
        crop_y2 = crop_y1 + crop_height
    
    # 从处理后的图像中裁剪
    cropped_img = processed_image[crop_y1:crop_y2, crop_x1:crop_x2]
...
def rotate_bound(image: np.ndarray, angle: float, center=None):
    ...
    if center is None:
        ...
        # 确保 center 是 Python 原生 float 类型
        cX = float(center[0])
        cY = float(center[1])
...
class InteractiveCropView(QGraphicsView):
    ...
    params_changed = pyqtSignal(float, float, float, float)
    ...
    def _update_crop_preview(self):
        ...
        for item in self.scene.items():
            if isinstance(item, QGraphicsTextItem) and item.toPlainText().endswith('°'):
                items_to_remove.append(item)
            if isinstance(item, QGraphicsLineItem) and item.pen().color().alpha() == 180 and item.pen().color().red() == 255:
                items_to_remove.append(item)
        ...
        # 预览时应用真实的旋转变换
        if self.image_item:
            if self.face_rotate_angle != 0:
                # 以人脸中心为旋转中心
                center = QPointF(self.face_info["center_x"], self.face_info["center_y"])
                self.image_item.setTransformOriginPoint(center)
                self.image_item.setRotation(self.face_rotate_angle)
                
                # 绘制旋转参考线（视觉辅助）
                rad = math.radians(self.face_rotate_angle)
                line_length = min(self.face_info["width"], self.face_info["height"]) // 2
                end_x = self.face_info["center_x"] + line_length * math.cos(rad)
                end_y = self.face_info["center_y"] - line_length * math.sin(rad)
                self.scene.addLine(
                    self.face_info["center_x"], self.face_info["center_y"],
                    end_x, end_y,
                    QPen(QColor(255, 100, 0, 180), 2, Qt.PenStyle.DashLine)
                )
                
                # 角度文字提示
                angle_text = self.scene.addText(f"{self.face_rotate_angle:+.1f}°")
                angle_text.setDefaultTextColor(QColor(255, 100, 0, 220))
                angle_text.setFont(QFont("Microsoft YaHei", 9, QFont.Weight.Bold))
                angle_text.setPos(self.face_info["center_x"] + 20, self.face_info["center_y"] - 25)
            else:
                # 无旋转时重置
                self.image_item.setRotation(0)
                self.image_item.setTransformOriginPoint(0, 0)
    ...
    def set_face_rotate_angle(self, angle: float):
        self.face_rotate_angle = angle
        self._update_crop_preview()
        # 发射信号通知主窗口参数变化（4 个参数）
    ...
    def wheelEvent(self, event: QWheelEvent):
        ...
        self._update_crop_preview()
        # 发射 4 个参数
        self.params_changed.emit(self.manual_offset_x, self.manual_offset_y, self.manual_scale, self.face_rotate_angle)
        event.accept()
    ...
    def reset_params(self):
        ...
        self.manual_offset_y = 0
        self.manual_scale = 1.0
        self.face_rotate_angle = 0  #重置旋转角度
        # 重置图像旋转
        if self.image_item:
            self.image_item.setRotation(0)
            self.image_item.setTransformOriginPoint(0, 0)
        self._update_crop_preview()
        # 发射 4 个参数
        self.params_changed.emit(0, 0, 1.0, 0.0)
...
class IDPhotoProcessThread(QThread):
    ...
    def run(self):
        try:
            ...
            #传递 face_rotate_angle 参数（旋转在裁剪前应用）
            standard_img, face_info = intelligent_crop_id_photo(
                ...
                face_rotate_angle=self.params.get("face_rotate_angle", 0)
            )
...
class HivisionIDPhotoGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        left_widget.setMinimumWidth(576)
        ...
        # 连接信号同时更新标签和预览
        def on_rotate_changed(value):
            self.face_rotate_label.setText(f"当前角度：{value}°")
            self.interactive_view.set_face_rotate_angle(value)
        
        self.face_rotate_dial.valueChanged.connect(on_rotate_changed)
        ...
        params_info_label = QLabel("当前参数：偏移 (0, 0) | 缩放 1.0x | 旋转 0.0°")
    ...
    # 接收 4 个参数
    def on_crop_params_changed(self, offset_x, offset_y, scale, rotate_angle=0):
        self.manual_offset_x = offset_x
        self.manual_offset_y = offset_y
        self.manual_scale = scale
        self.face_rotate_angle = rotate_angle
        self.params_info_label.setText(
            f"当前参数：偏移 ({offset_x:.1f}, {offset_y:.1f}) | 缩放 {scale:.2f}x | 旋转 {rotate_angle:+.1f}°"
        )
    ...
    def toggle_beauty(self, state):
        ...
        self.whitening_slider.setEnabled(enabled)
    ...
    def generate_photo(self):
        ...
        params = {
            ...
            "face_rotate_angle": self.face_rotate_angle,
            **face_params
        }
        self.btn_generate.setEnabled(False)
